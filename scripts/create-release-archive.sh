#!/usr/bin/env bash
# Create a self-contained Thruvera source archive, including vendored Pi source.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VERSION="${1:?Usage: scripts/create-release-archive.sh <version> [output-directory]}"
OUTPUT_DIR="${2:-${ROOT}/dist/release}"
ARCHIVE_NAME="thruvera-${VERSION}.tar.gz"

node "${ROOT}/scripts/verify-release-version.mjs" "${ROOT}" "${VERSION}"

[[ -f "${ROOT}/pi/package.json" ]] || {
	echo "Vendored Pi source is missing from this Thruvera checkout" >&2
	exit 1
}

STAGING="$(mktemp -d)"
trap 'rm -rf "${STAGING}"' EXIT
mkdir -p "${OUTPUT_DIR}" "${STAGING}/thruvera"
...
rm -f "${STAGING}/thruvera/core" "${STAGING}/thruvera"/core.*

mkdir -p "${STAGING}/thruvera/scripts"
for RELEASE_SCRIPT in clean-build-output.mjs install-media-dependencies.sh install.sh verify-release-version.mjs; do
	cp "${ROOT}/scripts/${RELEASE_SCRIPT}" "${STAGING}/thruvera/scripts/${RELEASE_SCRIPT}"
done
...
' "${STAGING}/thruvera/pi/packages/ai/tsconfig.build.json"

# Release archives omit Git metadata, so retain the exact release identity for runtime status checks.
printf '%s\n' "${VERSION}" > "${STAGING}/thruvera/RELEASE_VERSION"

tar -C "${STAGING}" -czf "${OUTPUT_DIR}/${ARCHIVE_NAME}" thruvera
