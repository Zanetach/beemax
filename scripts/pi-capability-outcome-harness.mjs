...
async function executeLivePiCapabilityTaskInRoot({ scenario, ranking, threshold, model, apiKey, createAgent, runId }, root) {
	...
	const tools = livePiTools({ candidates, descriptors, sourceByCapability, readSkills, completed, cognitionId: ranking.cognitionId });
...
export function summarizeLivePiOutcomeReceipts(receipts) {
	...
	let modelTurns = 0; let measuredTurns = 0; let totalInputTokens = 0; let totalOutputTokens = 0; let totalCostUsd = 0; let totalDurationMs = 0; let maxDurationMs = 0; let maxTokensPerCase = 0; let maxModelTurnsPerCase = 0;
	...
	return { cases, modelTurns, usageMeasurementRate: modelTurns ? measuredTurns / modelTurns : 0, totalInputTokens, totalOutputTokens, totalTokens: totalInputTokens + totalOutputTokens, averageTokensPerCase: cases ? (totalInputTokens + totalOutputTokens) / cases : 0, maxTokensPerCase, totalCostUsd: Math.round(totalCostUsd * 1e12) / 1e12, costEvidence: totalCostUsd > 0 ? "provider_reported" : "unpriced", averageDurationMs: cases ? totalDurationMs / cases : 0, maxDurationMs, maxModelTurnsPerCase };
}
...
function livePiTools({ candidates, descriptors, sourceByCapability, readSkills, completed, cognitionId }) {
	...
	const skillRead = Object.assign(withToolPolicy(defineTool({ name: "skill_read", label: "Read Skill", description: "Read the exact selected Skill before completing it", parameters: Type.Object({ name: Type.String() }), execute: async (_id, args) => { const candidate = skills.find((item) => item.name === args.name); if (!candidate) throw new Error("Unknown selected Skill"); readSkills.add(candidate.name); return { content: [{ type: "text", text: `loaded ${candidate.name}` }], details: { descriptor: { name: candidate.name }, skill: candidate.name, activatedTools: ["skill_complete"], skillLifecycleReceipt: { id: `receipt:live-pi:skill-read:${candidate.name}:${candidate.version}`, name: candidate.name, version: candidate.version, phase: "read", sourceTool: "skill_read" } } }; } }), READ_ONLY_TOOL_POLICY), { beemaxToolSpec: { kind: "skill", version: "eval:lifecycle", configured: true, health: "ready", authorized: true } });
	const skillComplete = Object.assign(withToolPolicy(defineTool({ name: "skill_complete", label: "Complete Skill", description: "Complete the Skill after reading it", parameters: Type.Object({ name: Type.String() }), execute: async (_id, args) => { const candidate = skills.find((item) => item.name === args.name); if (!candidate) throw new Error("Unknown selected Skill"); if (!readSkills.has(candidate.name)) throw new Error("Selected Skill must be read before completion"); completed.add(candidate.name); return { content: [{ type: "text", text: `completed ${candidate.name}` }], details: { skill: candidate.name, skillLifecycleReceipt: { id: `receipt:live-pi:skill-complete:${candidate.name}:${candidate.version}`, name: candidate.name, version: candidate.version, phase: "completed", sourceTool: "skill_complete" }, capabilityReceipt: { id: `receipt:live-pi:skill:${candidate.name}:${candidate.version}`, kind: "skill", name: candidate.name, version: candidate.version, sourceTool: "skill_complete" } } }; } }), READ_ONLY_TOOL_POLICY), { beemaxToolSpec: { kind: "skill", version: "eval:lifecycle", configured: true, health: "ready", authorized: true } });
