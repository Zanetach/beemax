...
15. 真实 E2E 已证明：未配置 `web_search` Provider 时，执行 Sub-Agent 会调用 `capability_discover`，切换 `agent_reach_search`，再用 `web_extract` 抓取；两个并行 Task 均经独立 Verification 后完成，Completion Delivery 最终返回核验报告，没有使用 evergreen 降级。
...
- 路由证据：`web_search` 明确返回未配置 Provider；两个隔离 Sub-Agent 均调用 `capability_discover`，随后 `agent_reach_search` 成功，再由 `web_extract` 核验官方 URL。
...
2. 外部“能力获取”目前只有公开信息研究和隔离 Skill Candidate 生命周期；尚无统一的 Tool/MCP Provider 候选解析、健康检查、配置建议和授权安装接口。
