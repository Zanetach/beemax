...
L4 结果驱动记忆、自主学习、情境化评估、自动降级与 Skill 持续演进的统一扩展规范见 [`BeeMax L4 Outcome-Driven Memory and Learning System`](../superpowers/specs/2026-07-18-beemax-l4-memory-learning-system.md)，落地契约见 [`L4 Memory Learning Architecture`](../architecture/l4-memory-learning-architecture.md)，迁移、灰度、真实测试和发布门槛见 [`L4 Memory Learning Rollout and Certification`](../operations/l4-memory-learning-rollout-and-certification.md)。它们复用本 PRD 的单一 Pi Runtime、Memory authority、Task/Effect/Verification authority 与 Profile rollout，不建立第二套执行或持久化系统。
