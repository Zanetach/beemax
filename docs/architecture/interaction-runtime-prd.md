...
**实施状态（2026-07-11）：** R1 已完成并经全仓测试验证。R2 已完成共享 Command Registry、实时 Footer、可恢复活动详情、结构化审批数据卡、会话/模型编号选择和单槽队列；`turn.steer` 在当前 Pi 运行时明确降级为 queue。Full 模式已具备 alternate-screen Workbench 帧（状态栏、转录区、活动卡、审批区和 Composer 指引），但尚未达到独立多栏 TUI（审批 Overlay、可搜索 picker 和真正多行 Composer）标准；这些保留为 R2 的后续工作，不能以“Full”名称掩盖。
