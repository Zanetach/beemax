...
**N3 实施状态：** Core 已提供强类型 `ToolPolicy`（风险、副作用、审批、可逆性、超时、重试次数、结果上限、影响说明）与 `ToolPolicyRegistry`。自定义一等 Tool 在进入 Pi 前统一包装：强制超时、仅无副作用工具允许有限重试、文本结果有界，并发布 requested/allowed/blocked/started/completed/failed 审计生命周期；审批卡直接消费同一 Policy，不再按工具名称推断风险。Pi 内建文件/执行工具当前使用显式内建策略并保留自身执行限制；后续批次继续将各能力工厂迁移为源码内声明 Policy，并持久化审计事件。
