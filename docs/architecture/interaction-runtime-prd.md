...
解决方向：在 `@beemax/core` 建立一个深模块 `InteractionRuntime`，将交互语义收敛为小接口的状态快照、事件流和动作；`beemax chat` 的 Full/Compact/Plain、Gateway、Web 仅作为 Adapter。
...
MVP 只覆盖 `InteractionRuntime + 自适应 beemax chat`；不包含移动节点、语音、Canvas 和复杂多人协作。
