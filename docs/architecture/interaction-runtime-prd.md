# Thruvera 统一交互运行时与终端工作台 PRD

| 项目属性 | 企业自研 × Agent 平台基础服务 |
| --- | --- |
| 重要性 | 高 |
| 紧迫性 | 高 |
| 需求方 | Thruvera 产品与工程团队 |
| PRD 编写人 | Thruvera |
| 提交日期 | 2026-07-11 |
...
| 变更时间 | 变更内容 | 修改人 | 版本 |
| --- | --- | --- | --- |
| 2026-07-11 | 初始长期架构设计 | Thruvera | v1.0 |
| 2026-07-11 | R1 完成；R2 已交付共享命令、实时状态、结构化审批、活动详情、Picker 与 queue/steer 降级 | Thruvera | v1.1 |
| 2026-07-11 | N1 打通 Pi 原生 steer/follow-up；旧 Runtime 保留确定性队列回退 | Thruvera | v1.2 |
| 2026-07-11 | N2 将模型自动回退收敛到 Core，并增加副作用护栏与统一可见事件 | Thruvera | v1.3 |
| 2026-07-11 | N3 建立 Tool Policy Registry 与统一执行治理第一阶段 | Thruvera | v1.4 |
| 2026-07-12 | N4 自主规划质量闸门拒绝无并行收益的伪 DAG | Thruvera | v1.5 |
| 2026-07-21 | 移除 Tool 审批事件、动作、Broker 与全部 Presenter UI；Tool 通过其余治理边界后直接执行 | Thruvera | v1.6 |
...
Thruvera 已具备 CLI、Gateway、会话、工具、子代理和结构化记忆闭环，但交互状态仍分散在 CLI/Gateway 的渲染逻辑与底层 Pi 事件之间。随着 Full Workbench、远程 Gateway、Web 控制台和移动端进入路线，继续复制命令与状态将造成行为分叉。

当前优先问题：

1. 工具、子代理和会话状态缺少统一、可复用的交互事件模型。
2. `thruvera chat` 需要同时覆盖 SSH/文本流与状态化工作台，不能要求用户在两个产品入口之间选择。
3. 未来 `thruvera chat` Full 模式、Gateway、Web 若各自解释 Runtime 状态，会产生权限、记忆 scope 与取消行为不一致。

解决方向：在 `@thruvera/core` 建立一个深模块 `InteractionRuntime`，将交互语义收敛为小接口的状态快照、事件流和动作；`thruvera chat` 的 Full/Compact/Plain、Gateway、Web 仅作为 Adapter。当前 I1 实现命名为 `InteractionEventAdapter`，只承担运行时事件转换与取消；完整的订阅、队列和跨端协调留在后续里程碑。
...
| 使用频率 | 每个交互式 Agent 回合 |
| 需求价值 | 以单一 `thruvera chat` 入口按环境自适应呈现，且不复制 Agent 逻辑 |

**主场景：本地长时间 Agent 工作台**

用户启动 Thruvera，在同一会话中查看模型、上下文、工具和子代理活动；Tool 在治理边界通过后直接执行；用户可取消、排队或引导运行中的任务，并在中断后恢复同一会话。

## 3、业务分析与系统调研

| 调研对象 | 可借鉴点 | 不直接复制的部分 |
| --- | --- | --- |
| OpenClaw | Gateway/local 连接到统一 TUI；持久 Footer；工具卡和会话选择器 | 不将 Thruvera 绑定到其 Gateway 协议或策略格式 |
| Hermes Agent | Classic CLI 与 TUI 共用 Runtime；busy input、详情分区 | 不复制其 Python/Node 双运行时实现，也不复制 modal 审批 |
| Thruvera 当前架构 | Core/Gateway 职责边界、Profile 隔离、Tool Governance 和记忆 policy 已在 Core 收敛 | 不允许 TUI/CLI 绕开 Core 做 prompt、记忆或授权决策 |
...
| Workbench 能力 | alternate screen、卡片、overlay、picker；由 `thruvera chat` 自动选择 | P1 |
| Remote Control | Gateway/Web 接入统一协议 | P1 |
| Cross-device | Web、移动节点、语音与 Canvas | P2 |

MVP 先覆盖 `InteractionEventAdapter + 自适应 thruvera chat`；不包含移动节点、语音、Canvas 和复杂多人协作。

## 6、项目范围

| 系统 | 关系 | 本期影响 |
| --- | --- | --- |
| `@thruvera/core` | 主体 | 新增交互运行时与状态机 |
| `apps/cli` | Adapter | `thruvera chat` 的 Full/Compact/Plain 渲染与输入适配 |
...
| 风险 | 影响 | 应对 |
| --- | --- | --- |
| 直接暴露 Pi 事件 | UI 与底层耦合，未来替换困难 | Core 转换为 Thruvera 语义事件 |
| Chat 各模式命令分叉 | 用户行为与测试不一致 | Command registry 只在 Core 定义一次 |
| 全屏工作台覆盖 SSH 场景 | 运维可用性下降 | 同一 `thruvera chat` 自动降级 Plain Presenter，支持 `--plain` |
...
| 术语 | 定义 |
| --- | --- |
| Interaction Runtime | Thruvera 的交互状态机、事件流和动作入口 |
...
搜索、浏览器、记忆、文件、日历、邮件、渠道、媒体与自动化均为 Thruvera 的一等能力。每项能力必须拥有：稳定输入/输出 schema、明确风险等级、可关联 Profile/Scope/Turn 的审计事件，以及由 Core 执行的 Tool Governance。Presenter、Skill 与渠道不得以脚本拼接替代这些契约。
...
#### 10.2.3 `thruvera chat` 自适应 Presenter
...
`thruvera tui` 如保留，仅作为 `thruvera chat --full` 的兼容别名；产品文档、帮助和 onboarding 只推荐 `thruvera chat`。
...
Gateway 只把渠道消息归一化为 Action，并把 Event 降级渲染为卡片/文本；Web 与 `thruvera chat` Full 模式可消费完整 Event。任何界面都不得自行改变模型、prompt、memory scope、Tool Governance 决定或工具 policy。
...
| I3 Full Workbench | 内部开发者 | `thruvera chat` 的 picker、overlay、工具卡、恢复 | Compact/Plain 自动回退 |
...
R1 先完成，未通过其验收不进入新增渠道或语音开发；这保证 Thruvera 不会以更多入口扩大不一致行为。
...
| 编号 | 待决事项 | 负责人 | 状态 |
| --- | --- | --- | --- |
| TBD-1 | Full Workbench 技术选型 | Thruvera | 已决策：复用 `@earendil-works/pi-tui`，Plain/Compact 保留独立降级 |
| TBD-2 | 远程 Interaction Protocol 与认证 | Thruvera | 已决策：v1 JSON/HTTP，Bearer 映射精确 Scope；事件按 sequence 恢复，后续可增加 WebSocket transport |
| TBD-3 | queue/steer 兼容性与上限 | Thruvera | 已决策：Pi Runtime 使用原生 steer/follow-up；旧 Runtime 保留单槽 queue，steer 不支持时明确降级并替换已有输入 |
| TBD-4 | Tool 直接执行治理 | Thruvera | 已决策：无人工审批；Enterprise `require_approval` fail-closed，其余 Tool 通过硬边界后直接执行 |
| TBD-5 | 多用户 Profile 管理边界 | Thruvera | 已决策：当前 Profile 为单所有者隔离单元；不提供管理员跨用户记忆/会话读取，未来多租户另立治理 PRD |
