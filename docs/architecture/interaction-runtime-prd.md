...
**实施状态（2026-07-11）：** R1 已完成并经全仓测试验证：事件信封、原子取消、语义审批与有界 `actionId` 幂等已在 Core 收敛。R2 已完成共享 Command Registry、实时 Footer、可恢复活动详情、结构化审批数据卡、会话/模型搜索选择、单槽队列、Pi 多行 Composer 与 Sub-Agent Inspector；`turn.steer` 在当前 Pi 运行时明确降级为 queue。Full 模式已具备 alternate-screen Workbench（状态栏、转录区、活动卡、子代理卡、审批区、搜索 Picker、可编辑 Composer 与键盘审批 Overlay）；审批选择统一提交 Core `approval.decide` Action，子代理按当前 scope 刷新并由 `/stop` 原子级联取消。
