...
**Credential Vault 第一阶段（2026-07-12）：** Core 已提供 owner-scoped 的加密 Vault 与不透明 Credential Ref；公开接口只允许列出非秘密元数据，明文仅能进入可信 Capability 的短生命周期 consumer callback，错误密钥、错误 owner 与删除后的引用均 fail closed，访问审计不含标签、用途或秘密。Profile 密钥托管、CLI 管理入口以及浏览器/邮件的直接注入属于下一纵切，完成前不提供会向模型返回明文的通用 `credential_get` Tool。
