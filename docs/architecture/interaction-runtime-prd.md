...
**N3 实施状态：** Core 已提供强类型 `ToolPolicy`（风险、副作用、审批、可逆性、超时、重试次数、结果上限、影响说明）与 `ToolPolicyRegistry`。自定义一等 Tool 在进入 Pi 前统一包装：强制超时、仅无副作用工具允许有限重试、文本结果有界，并发布 requested/allowed/blocked/started/completed/failed 审计生命周期；审批卡直接消费同一 Policy，不再按工具名称推断风险。每个 Profile 现将不含参数、结果和内容的审计事件写入权限受限、有界的 `tool-audit.jsonl`。浏览器、记忆、自动化、Skill、子代理、媒体、Feishu 与 MCP 均已迁移为源码内 Policy：稳定非敏感记忆可自动写入；日程、Skill 与外部变更继续审批；子代理编排为无审批、不重试的有界本地操作；MCP 根据服务端 read-only/destructive 注解生成策略。Pi 内建文件/执行工具使用显式内建策略并保留自身执行限制。新增产品能力必须在能力源码处声明 Policy，不再扩展 CLI 工具名审批清单。
...
**Credential Vault 第六阶段（2026-07-12）：** Core 已提供 owner-scoped 的加密 Vault 与不透明 Credential Ref；公开接口只允许列出非秘密元数据，错误密钥、错误 owner 与删除后的引用均 fail closed。Profile 生命周期在受保护的 `state/credential-vault.key` 生成或安全补齐独立主密钥，使备份与删除归档不会把密文和密钥拆散；`beemax credentials` 提供不经 argv 的添加、列表、原 Ref 轮换和删除入口，轮换立即替换旧秘密但保持记忆、任务和自动化引用稳定。doctor 校验密钥与密文，内容无关访问事件写入权限受限的有界审计。一等 `browser_fill_credential` 只接收 selector 与 Credential Ref；`browser_generate_credential` 在 Core 本地生成高熵密码、存入 Vault 并直接填入网页，只返回 Ref，填充失败则删除新建凭据。Gateway 与 CLI 的 Vault 写入由短生命周期、token-fenced 的跨进程写入租约串行化；活跃租约 fail closed，进程崩溃留下的陈旧租约可回收，避免密文最后写入覆盖。Tool 结果、详情、审批与普通审计均不含秘密。邮件等后续 Capability 复用同一注入边界；始终不提供会向模型返回明文的通用 `credential_get` Tool。
...
**实施状态（2026-07-11）：** R1 已完成并经全仓测试验证：事件信封、原子取消、语义审批、会话 open/reset/compact 与有界 `actionId` 幂等已在 Core 收敛；CLI 和 Gateway 的 Profile Control 也走同一 Action 路径。R2 已完成共享 Command Registry、实时 Footer、可恢复活动详情、结构化审批数据卡、会话/模型搜索选择、Pi 多行 Composer 与 Sub-Agent Inspector。N1 已将 `turn.steer` 和 `turn.queue` 分别接入 Pi 原生 steer/follow-up；CLI 运行中普通输入采用 follow-up、`/steer` 显式中途引导，Gateway 同会话第二条消息直接进入原生 follow-up；旧 Runtime 或不可用会话仍保留单槽队列回退。N2 已把模型回退从 CLI 收回 Core：Pi 同模型重试耗尽后，仅对瞬时网络/限流/5xx 故障尝试有界候选模型；若已输出文本或启动任何工具则禁止自动重放，保留显式 `/retry`；CLI、Full Workbench 与 Gateway 统一展示 `model.fallback` 事件并记录无内容遥测。Full 模式已具备 alternate-screen Workbench（状态栏、转录区、活动卡、子代理卡、审批区、搜索 Picker、可编辑 Composer 与键盘审批 Overlay）；审批选择统一提交 Core `approval.decide` Action，子代理按当前 scope 刷新并由 `/stop` 原子级联取消。R3 已具备版本化控制协议、运行时 JSON 校验、精确 scope 校验、隐私受控事件恢复，以及默认仅 loopback 的 Bearer 认证 HTTP transport。R4 已打通首条真实媒体管道：Feishu 图片/音频/文件以 25MB BeeMax 上限流式下载到权限收紧的临时目录，Dispatcher 负责生命周期回收；图片作为 Pi 原生视觉附件传入，其他附件以非可信清单交给一等工具，媒体字节不进入提示文本、日志或遥测。语音转写/输出、Canvas 与设备节点尚未实现。R5 基础埋点已在 Core 记录无内容的回合、审批、队列和重连质量。公网 Web 部署与外部渠道 adapter 仍按账号可用性推进。
...
| TBD-4 | 审批风险与长期策略 | BeeMax | 已决策：低/中/高风险卡；once/session/deny 为交互决策，长期 allowlist 仅由 Profile 管理入口配置 |
