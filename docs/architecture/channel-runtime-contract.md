# Thruvera Channel Runtime Contract
...
非敏感声明使用 `gateway.channels[]`，Credential Secret 不得出现在 adapter settings 或普通 `ThruveraConfig`。`credentialRef` 只指向 Profile 受保护的 Secret 来源；当前内置 adapter 使用 `profile-env:feishu` 和 `profile-env:telegram`，并只通过可信 Adapter/诊断边界的 callback-only consumer 即时消费；Adapter 只保留这个窄化 consumer，不保留明文 settings 副本。未知 adapter ID、重复活动平台、无启用渠道、Credential/配置错误均 fail closed。已正确配置的渠道如果只因临时网络或平台故障在启动瞬间全部离线，Profile Runtime 可以保持运行：ChannelHost 继续监督重连，Scheduler 继续产生 durable work，Delivery Outbox 等渠道恢复后再投递。离线 adapter 不得被报告为 connected，也不能接收入站流量。
...
- 将 channel-neutral progress、answer、media 与 delivery outcome 呈现为平台格式；Tool 调用不创建审批交互；
