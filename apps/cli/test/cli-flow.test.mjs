...
test("CLI supports init, model setup, Feishu channel setup, listing, and safe deletion", async () => {
	...
	assert.match(run(["init", "--profile", "personal"]), /Created Thruvera Agent 'personal'/);
	...
	assert.match(run(["capabilities", "install", "agent-reach", "--profile", "personal"]), /Installed Thruvera-native Agent Reach/);
	...
	assert.throws(
		...
		/differs from Thruvera's packaged revision/,
	);
	await writeFile(agentReachPath, packagedAgentReach);
	const profileEnvPath = join(home, "profiles", "personal", ".env");
	await writeFile(profileEnvPath, `${await readFile(profileEnvPath, "utf8")}THRUVERA_CHROME_EXECUTABLE=${JSON.stringify(join(root, "missing-chrome"))}\n`, { mode: 0o600 });
	...
	assert.throws(
		() => run(["capabilities", "start", "pi-web-access", "--profile", "personal"], { THRUVERA_CHROME_EXECUTABLE: join(root, "trusted-host-missing-chrome") }),
	...
	assert.match(run(["model", "set", "openrouter", "openai/gpt-5.2", "--profile", "personal", "--non-interactive"], {
		THRUVERA_API_KEY: "model-key",
	}), /Configured openrouter\/openai\/gpt-5.2/);
	assert.match(run(["model", "set", "custom", "private-model", "--profile", "personal", "--non-interactive"], {
		THRUVERA_API_KEY: "custom-key",
	}), /Configured custom\/private-model/);
	const backupCredentialOutput = run(["credentials", "add", "--profile", "personal", "--label", "Backup account", "--purpose", "backup login", "--non-interactive"], { THRUVERA_CREDENTIAL_SECRET: "backup-private-value" });
	...
	const storedCredential = run(["credentials", "add", "--profile", "personal", "--label", "Example account", "--purpose", "example.com login", "--non-interactive"], { THRUVERA_CREDENTIAL_SECRET: "correct-horse-battery-staple" });
	...
	assert.match(run(["credentials", "rotate", credentialRef, "--profile", "personal", "--non-interactive"], { THRUVERA_CREDENTIAL_SECRET: "rotated-private-value" }), new RegExp(`Rotated Credential Ref ${credentialRef}`));
	...
	assert.equal(run(["profile", "list", "--home", home, "--root", root], {
		THRUVERA_HOME: join(home, "wrong"),
		THRUVERA_ROOT: join(root, "wrong"),
	}).trim(), "personal");
	...
	assert.equal(run(["agent", "list"]).trim(), "personal");
	const serviceName = process.platform === "darwin" ? "com.thruvera.agent.personal" : process.platform === "linux" ? "thruvera@personal.service" : "thruvera:personal";
...
test("fact-sensitive chat questions receive task and installed-version facts, not restored transcript claims", async () => {
	...
	try {
		...
		assert.match(taskLedgerContextForQuestion(store, "What Thruvera version is installed?"), /installed_version=/);
...
test("unified setup configures an isolated Profile and Feishu gateway non-interactively", async () => {
	...
	const home = await mkdtemp(join(tmpdir(), "beemax-home-"));
	const previousRoot = process.env.THRUVERA_ROOT;
	const previousHome = process.env.THRUVERA_HOME;
	const isolatedEnvKeys = ["THRUVERA_PROFILE", "THRUVERA_PROVIDER", "THRUVERA_MODEL", "THRUVERA_API_KEY", "THRUVERA_DB_PATH", "THRUVERA_MCP_CONFIG", "THRUVERA_AGENT_DIR", "THRUVERA_CWD", "FEISHU_APP_ID", "FEISHU_APP_SECRET", "FEISHU_ALLOWED_USERS"];
	...
	try {
		assert.equal(await runSetup({
			...
			apiKey: "model-secret",
			soul: "You are the dedicated Thruvera operations assistant.",
			appId: "cli_setup",
			appSecret: "feishu-secret",
			allowedUsers: ["ou_setup"],
		}, {
			probe: async () => ({ botName: "Thruvera Setup Bot", botOpenId: "ou_bot" }),
			doctor: async () => true,
		}), true);
		...
		assert.equal(await runSetup({ profile: "assistant", nonInteractive: true }, {
			probe: async () => ({ botName: "Thruvera Setup Bot" }),
			doctor: async () => true,
		}), true);
		...
		}, { probe: async () => ({ botName: "Thruvera Gateway Bot" }) }), true);
		configured = loadConfig(join(home, "profiles", "assistant", "config.yaml"), "assistant");
	} finally {
		console.log = previousLog;
		if (previousRoot === undefined) delete process.env.THRUVERA_ROOT; else process.env.THRUVERA_ROOT = previousRoot;
		if (previousHome === undefined) delete process.env.THRUVERA_HOME; else process.env.THRUVERA_HOME = previousHome;
	...
	assert.match(output, /Thruvera setup complete/);
	const profileHome = join(home, "profiles", "assistant");
	assert.equal((await readFile(join(profileHome, "SOUL.md"), "utf8")).trim(), "You are the dedicated Thruvera operations assistant.");
...
test("setup keeps the generated SOUL unless the user explicitly supplies a custom identity", async () => {
	...
	try {
		...
		assert.match(await readFile(soulPath, "utf8"), /# Thruvera/);
		await runSetup({
		...
		process.env.THRUVERA_ROOT = previousRoot;
		process.env.THRUVERA_HOME = previousHome;
	}
});
...
test("Feishu Gateway manual fallback is guided and keeps safe WebSocket defaults", async () => {
	...
	try {
		...
		assert.match(output, /thruvera gateway run --profile guided/);
