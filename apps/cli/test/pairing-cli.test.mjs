...
import { PairingStore } from "@beemax/gateway";
...
test("pairing CLI lists, approves, and revokes a Profile-scoped Feishu user", async () => {
	...
	try {
		...
		const run = (...args) => execFileSync(process.execPath, [cli, ...args, "--profile", "paired"], { encoding: "utf8", env: { ...process.env, BEEMAX_HOME: home } });
