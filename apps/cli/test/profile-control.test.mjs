...
test("shared /status reports Profile task admission capacity on every channel", async () => {
	...
	const control = createProfileControlHandler(runtime, config, undefined, () => ({
		ingress: { active: 7, activeConversations: 4, maxActive: 100, maxActivePerConversation: 20, rejected: 2 },
		profileHost: { state: "degraded", acceptingInteractions: true, lifecycleRejected: 1, degradedReasons: ["Channel Instance telegram-main is failed"], ingress: { active: 7, activeConversations: 4, maxActive: 100, maxActivePerConversation: 20, rejected: 2 } },
	...
	assert.match(result.message, /Ingress: active=7\/100; conversations=4; per-conversation=20; rejected=2/);
	assert.match(result.message, /Profile Host: degraded; accepting=true; lifecycle-rejected=1; degraded=Channel Instance telegram-main is failed/);
