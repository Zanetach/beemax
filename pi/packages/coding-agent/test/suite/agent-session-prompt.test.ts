...
describe("AgentSession prompt characterization", () => {
	...
	it("summarizes only results from tools explicitly trusted for turn-scoped persistence", async () => {
		const makeTool = (name: string, trusted: boolean): AgentTool => Object.assign({
			name, label: name, description: name, parameters: Type.Object({}),
			execute: async () => ({ content: [{ type: "text" as const, text: `secret-${name}` }], details: { beemaxPersistence: { mode: "summary" }, root: "/private/path", sha256: "abc" } }),
		}, trusted ? { persistResultAsSummary: true } : {});
		const harness = await createHarness({ tools: [makeTool("trusted", true), makeTool("untrusted", false)] }); harnesses.push(harness);
		harness.setResponses([fauxAssistantMessage(fauxToolCall("trusted", {}), { stopReason: "toolUse" }), fauxAssistantMessage(fauxToolCall("untrusted", {}), { stopReason: "toolUse" }), fauxAssistantMessage("done")]);
		await harness.session.prompt("start");
		const persisted = harness.sessionManager.getEntries().filter((entry) => entry.type === "message" && entry.message.role === "toolResult").map((entry) => entry.message);
		expect(JSON.stringify(persisted[0])).toContain("Turn-scoped Skill context omitted"); expect(JSON.stringify(persisted[0])).not.toContain("/private/path");
		expect(JSON.stringify(persisted[1])).toContain("secret-untrusted");
	});
