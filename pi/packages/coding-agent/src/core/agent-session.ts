...
type PersistedAgentMessage = Extract<AgentMessage, { role: "user" | "assistant" | "toolResult" }>;
function persistenceSafeMessage(message: PersistedAgentMessage, summarize: boolean): PersistedAgentMessage {
	if (message.role !== "toolResult" || !summarize) return message;
	const details = message.details && typeof message.details === "object" ? message.details as Record<string, unknown> : {};
	const state = details.state && typeof details.state === "object" ? details.state as Record<string, unknown> : undefined;
	const descriptor = details.descriptor && typeof details.descriptor === "object" ? details.descriptor as Record<string, unknown> : undefined;
	const safeDetails = { descriptor: descriptor ? { name: descriptor.name, description: descriptor.description, sha256: descriptor.sha256, priority: descriptor.priority } : undefined, state, path: typeof details.path === "string" ? details.path : undefined, sha256: typeof details.sha256 === "string" ? details.sha256 : undefined, bytes: typeof details.bytes === "number" ? details.bytes : undefined, kind: details.kind };
	return { ...message, content: [{ type: "text", text: "[Turn-scoped Skill context omitted; versioned execution metadata retained.]" }], details: safeDetails };
}
...
export class AgentSession {
	...
	private _handleAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "custom") {
				...
				this.sessionManager.appendMessage(persistenceSafeMessage(event.message, event.message.role === "toolResult" && this._toolDefinitions.get(event.message.toolName)?.definition.persistResultAsSummary === true));
			}
	...
	/**
	 * Continue the most recent failed assistant step with another model without
	 * duplicating the user message. Intended for product-level failover policy.
	 */
	async retryWithModel(model: Model<any>): Promise<boolean> {
