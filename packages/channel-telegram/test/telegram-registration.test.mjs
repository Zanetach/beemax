...
test("Telegram registration creates independent Channel Instances from settings and Credential Refs", () => {
	...
	const registration = createTelegramAdapterRegistration({
		...
		consumeCredentials: (instance, consumer) => { requested.push(`${instance.id}:${instance.credentialRef}`); return consumer({ botToken: `token-${instance.id}` }); },
	});
	...
	assert.deepEqual(requested, ["team-a:profile-env:channel:team-a", "team-a:profile-env:channel:team-a", "team-b:profile-env:channel:team-b", "team-b:profile-env:channel:team-b"]);
});

test("Telegram registration fails closed on missing credentials and invalid instance settings", () => {
	const registration = createTelegramAdapterRegistration({ defaults: { allowedUsers: [], allowedChats: [], allowAllUsers: false }, consumeCredentials: (instance, consumer) => instance.id === "missing" ? undefined : consumer({ botToken: "token" }) });
