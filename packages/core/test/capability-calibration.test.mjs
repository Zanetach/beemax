...
test("Capability calibration measures ranking, activation, verified completion, latency, tokens, and cost from correlated outcomes", () => {
	const report = evaluateCapabilityCalibration({
		...
		observations: [
			{ caseId: "research", cognitionId: "cap:1", ranked: [{ name: "web_search", confidence: 0.95 }], activatedCapabilities: ["web_search"], outcome: "accepted", latencyMs: 120, inputTokens: 40, outputTokens: 10, costUsd: 0.02 },
			{ caseId: "meeting", cognitionId: "cap:2", ranked: [{ name: "meeting_schedule", confidence: 0.9 }], activatedCapabilities: ["meeting_schedule"], outcome: "accepted", latencyMs: 180, inputTokens: 50, outputTokens: 15, costUsd: 0.03 },
			{ caseId: "direct-chat", cognitionId: "cap:3", ranked: [], activatedCapabilities: [], outcome: "accepted", latencyMs: 60, inputTokens: 20, outputTokens: 10, costUsd: 0.01 },
		],
	});
	assert.deepEqual(report.metrics, {
		...
		averageLatencyMs: 120, totalTokens: 145, averageTokens: 145 / 3, totalCostUsd: 0.06,
	});
...
test("Capability calibration rejects candidates with material latency, token, or cost regressions", () => {
	const observations = [
		{ caseId: "research", cognitionId: "cap:r1", ranked: [{ name: "web_search", confidence: 0.95 }], activatedCapabilities: ["web_search"], outcome: "accepted", latencyMs: 100, inputTokens: 10, outputTokens: 10, costUsd: 0.01 },
		{ caseId: "meeting", cognitionId: "cap:r2", ranked: [{ name: "meeting_schedule", confidence: 0.95 }], activatedCapabilities: ["meeting_schedule"], outcome: "accepted", latencyMs: 100, inputTokens: 10, outputTokens: 10, costUsd: 0.01 },
		{ caseId: "direct-chat", cognitionId: "cap:r3", ranked: [], activatedCapabilities: [], outcome: "accepted", latencyMs: 100, inputTokens: 10, outputTokens: 10, costUsd: 0.01 },
	];
