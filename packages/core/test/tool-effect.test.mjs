...
test("effect journal records a content-free mutation lifecycle and committed receipt", () => {
	...
	try {
		...
		const id = effects.begin({ source, taskId: "turn-1", toolCallId: "call-1", toolName: "write", policy: { ...MUTATING_TOOL_POLICY, sideEffect: "local" } });
		assert.ok(id);
		...
		assert.deepEqual(records.at(-1).receipt, { status: "committed", occurredAt: records.at(-1).at });
