...
/** Proposal-only cognition is intentionally not admissible as a Thruvera Agent Runtime port. */
...
export const WORK_CONTRACT_SYSTEM_PROMPT = `You propose a structured Work Contract for one Thruvera Turn. This is bounded, Tool-free cognition, not an Agent loop.
...
export const SEMANTIC_INVENTORY_SYSTEM_PROMPT = `Independently inventory the complete semantics of one Thruvera Raw Request. This is bounded, Tool-free cognition, not an Agent loop and not a review of another model response.
...
const COMPACT_SEMANTIC_INVENTORY_SYSTEM_PROMPT = `Independently inventory one Thruvera Raw Request for deterministic compilation. Return JSON only, with no reasoning or markdown.
