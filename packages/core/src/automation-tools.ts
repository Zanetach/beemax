...
export function createAutomationTools(store: AutomationStore, source: BeeMaxRuntimeSource, wakeScheduler: () => void): ToolDefinition[] {
	...
	const tools = [
		...
		defineTool({ name: "schedule_update", label: "Update Schedule", description: "Update an owned Schedule and recompute its next occurrence when timing changes. Requires approval.", parameters: Type.Object({ id: Type.String(), name: Type.Optional(Type.String({ minLength: 1, maxLength: 120 })), kind: Type.Optional(StringEnum(["reminder", "agent"] as const)), scheduleKind: Type.Optional(StringEnum(["at", "every", "cron"] as const)), schedule: Type.Optional(Type.String()), text: Type.Optional(Type.String({ minLength: 1, maxLength: 20_000 })), timezone: Type.Optional(Type.String()), maxAttempts: Type.Optional(Type.Integer({ minimum: 1, maximum: 20 })), misfirePolicy: Type.Optional(StringEnum(["skip", "run_once"] as const)), misfireGraceMs: Type.Optional(Type.Integer({ minimum: 0, maximum: 604_800_000 })) }), execute: async (_id, params) => {
