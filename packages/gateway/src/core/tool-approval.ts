/**
 * Text-based tool approval broker for Feishu long-connection deployments.
 *
 * Card button callbacks require a public webhook, so local Linux deployments
 * use replies:
 *   1 / 允许一次
 *   2 / 本会话允许
 *   3 / 拒绝
 *
 * Approval replies are consumed before the per-session dispatcher lock, which
 * lets a tool call wait without deadlocking the active agent turn.
 */

import type { InboundMessage, SessionSource } from "./types.ts";
...
export class ToolApprovalBroker {
	...
	async authorize(request: ToolApprovalRequest, signal?: AbortSignal): Promise<ToolApprovalDecision> {
		...
		try {
			await this.sendPrompt(request.source, renderApprovalPrompt(request));
		} catch (error) {
			this.finish(sourceKey, {
				allowed: false,
				reason: `Could not deliver approval request: ${error instanceof Error ? error.message : String(error)}`,
			});
		}
		return decision;
	}

	/** Returns true when the inbound message was consumed as an approval reply. */
	async handleMessage(message: InboundMessage): Promise<boolean> {
		const sourceKey = sessionKeyForSource(message.source);
		...
		if (!choice) {
			await this.sendPrompt(
				message.source,
				"存在待审批工具调用。请回复：1（允许一次）、2（本会话允许）或 3（拒绝）。",
			);
			return true;
		}
		...
		if (choice === "session") {
			...
			await this.sendPrompt(message.source, `已允许本会话继续使用工具 \`${pending.toolName}\`。`);
		} else if (choice === "once") {
			this.finish(sourceKey, { allowed: true });
			await this.sendPrompt(message.source, `已允许本次工具调用 \`${pending.toolName}\`。`);
		} else {
			this.finish(sourceKey, { allowed: false, reason: "User denied the tool call" });
			await this.sendPrompt(message.source, `已拒绝工具调用 \`${pending.toolName}\`。`);
		}
		return true;
	}
	...
	dispose(): void {
		for (const key of [...this.pending.keys()]) {
			this.finish(key, { allowed: false, reason: "Gateway is shutting down" });
		}
...
function formatArgs(args: unknown): string {
	let rendered: string;
	try {
		rendered = JSON.stringify(redact(args), null, 2);
	} catch {
