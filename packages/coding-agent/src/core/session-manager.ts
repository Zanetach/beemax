...
const TURN_SCOPED_SKILL_TOOLS = new Set(["skill_activate", "skill_read", "skill_resource_read"]);

/** Remove Skill instructions written by BeeMax versions predating turn-scoped persistence. */
function migrateTurnScopedSkillResults(entries: FileEntry[]): boolean {
	let changed = false;
	for (const entry of entries) {
		if (entry.type !== "message" || entry.message.role !== "toolResult" || !TURN_SCOPED_SKILL_TOOLS.has(entry.message.toolName)) continue;
		const summary = "[Turn-scoped Skill context omitted; versioned execution metadata retained.]";
		const content = entry.message.content;
		if (content.length === 1 && content[0]?.type === "text" && content[0].text === summary) continue;
		entry.message = { ...entry.message, content: [{ type: "text", text: summary }] };
		changed = true;
	}
	return changed;
}
...
export class SessionManager {
	...
	setSessionFile(sessionFile: string): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			const versionMigrated = migrateToCurrentVersion(this.fileEntries);
			const skillResultsMigrated = migrateTurnScopedSkillResults(this.fileEntries);
			if (versionMigrated || skillResultsMigrated) {
				this._rewriteFile();
			}
	...
	private _rewriteFile(): void {
		...
		try {
			...
			try { unlinkSync(temporary); } catch { /* Best-effort cleanup after a failed atomic replace. */ }
			throw error;
		}
	}
