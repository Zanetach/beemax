...
![Thruvera Agent 将作用域上下文和持久记忆转化为受治理、可验证的执行](docs/assets/thruvera-agent-runtime.png)

Thruvera 是一个 Agent 产品，只有一个由 Core 拥有的 Pi 执行循环。它可以在本地终端中对话，也可以通过同一个 Profile Gateway 接入飞书/Lark 和 Telegram；长期任务可以跨重启继续，图片可以通过模型原生视觉或 OCR 理解。

所有入口都执行同一套 Profile 作用域策略。

Thruvera 不在代码里固化订单、工单、合同等客户业务对象。未知业务词汇通过 Work Context、证据、已配置能力和企业策略进入系统，而不是被强制映射到固定业务本体。
...
Pi 负责理解任务、动态执行、模型交互、Tool、Session 事件和实时压缩。Thruvera Core 在 Pi 外增加产品语义：渐进能力披露、作用域、Sandbox、Tool Policy、持久责任、Effect 权威、恢复、Verification 和交付。Work Contract 只治理持久或后台责任，不是普通交互在进入主模型前必须经过的分类步骤。
...
引导安装器会解析 GitHub 最新稳定 Release，下载包含 Thruvera 和 vendored Pi 源码的校验和归档。应用文件安装到 `~/.thruvera/app`，命令安装到 `~/.local/bin`。
...
Ubuntu 和 macOS 安装过程会发现或安装 Caddy 与 Tesseract OCR。Ubuntu 还会安装 Noto CJK 字体，保证中文 HTML/PDF 报告中的文字可见且可提取。只有宿主镜像自行管理全部这些依赖时，才应设置 `THRUVERA_INSTALL_MEDIA_DEPS=0`。

每个 Profile 默认启用独立的 Caddy Artifact Site，为通过完整性校验的 HTML、PDF 和 Word 输出提供链接。`thruvera doctor` 与 Gateway 共用可信宿主命令解析器，并在启动前验证 `caddy version`。仍可通过 `gateway.artifactSite.enabled: false` 显式关闭。Caddy 只接收宿主启动所需的非敏感白名单变量，不接收 Profile 环境或 Secret；HOME、XDG 和临时状态始终留在当前 Profile 的私有 runtime 目录。
...
向导会配置 Profile 身份、模型、凭据、工作区、Skill 和本地就绪状态。Secret 通过安全提示输入，存放在 YAML 之外。
...
本地对话和渠道 Gateway 使用相同的 Profile、Memory、Skill、Pi Runtime、治理规则和持久工作图。
...
flowchart TB
    ...
    subgraph Profile["apps/cli · createProfileRuntime · 每个 Profile 唯一装配根"]
        ...
        Memory["@thruvera/memory · Profile SQLite<br/>Claim · Episode · Task Ledger · Verification<br/>学习谱系 · 评估 · 投影"]
        Automation["@thruvera/automation<br/>Schedule · Occurrence · 租约 · 交付状态"]
...
[`apps/cli/src/runtime-composition.ts`](apps/cli/src/runtime-composition.ts) 是应用装配入口，不是第二个 Agent Runtime。它为 CLI 或 Gateway 创建同一套 Profile Runtime。[`apps/cli/src/profile-work-runtime.ts`](apps/cli/src/profile-work-runtime.ts) 绑定同一个 Task Ledger、Effect 权威、execution Trace、恢复服务、Verification 和 Memory Learning Kernel。

`@thruvera/core` 是唯一 Agent Runtime 边界，Pi 是 Core 私有的执行底座。Gateway 只负责可信渠道认证、路由、生命周期、呈现和交付，不负责选择模型、组装 Prompt、召回 Memory、授权 Tool 或决定 Agent 工作。能力包只能通过 Core-owned Tool Contract 接入，不能绕过 Profile 作用域、Sandbox、Enterprise Policy 或 Effect 治理。

能力包通过 Core 使用 Pi 原语。CLI 呈现层可以使用 `pi-tui`，但不拥有 Agent 执行。详见 [Core/Gateway 所有权契约](docs/architecture/core-gateway-boundaries.md)。

## Profile 与配置

每个 Profile 都是 `~/.thruvera/profiles/<name>/` 下相互隔离的 Agent Home。
...
可用 `THRUVERA_HOME` 迁移所有 Profile Home。旧的仓库内 Profile 仍可读取，也可以使用 `thruvera profile migrate <name>` 无损复制。

更名前的 BeeMax 安装可以直接继续使用：当 `~/.thruvera` 不存在时，Thruvera 会复用已有的 `~/.beemax`；`BEEMAX_*` 环境变量和 `beemax` 命令保留为兼容别名。两套环境变量同时存在时以 `THRUVERA_*` 为准。持久化数据中的 `beemax.*` schema 标识作为稳定磁盘协议命名空间保留，不进行破坏性改写。
...
API Key 应保存在 Profile `.env` 中，或通过 `thruvera setup` 输入。Thruvera 会拒绝从命令行参数传入模型或凭据 Secret。
...
每个新 Profile 默认启用 `standard-web` 能力包，并且只预授权 Thruvera 固定版本的 `exa-mcporter` Adapter。变更型 `capability_acquire` Tool 在 Provider、Profile 作用域、Enterprise Policy 和 Effect 边界全部通过后直接执行，且必须在健康探测返回证据后才恢复原 Objective。管理员仍可在 Profile 中关闭自动获取或替换精确 allowlist：
...
等价环境变量为 `THRUVERA_PROVIDER_INSTALLATION_ENABLED=true` 和 `THRUVERA_PROVIDER_INSTALLATION_ALLOW=exa-mcporter`；将前者设为 `false` 可显式关闭。Thruvera 当前提供固定版本的 Exa/mcporter Adapter 以恢复 `web_search`；不会接受任意包名或模型生成的 shell 命令。安装依赖来自 SHA-256 验证的 `package-lock`，禁用包生命周期脚本，并在 Profile 私有目录中原子发布。配置缺失、权限拒绝、安装不健康或结果未知都会 fail closed。
...
Thruvera 将聊天历史与持久组织证据分离：

- 每个 Profile 的 SQLite 是唯一语义 Memory 权威；Task、Effect、Verification 和交付 Ledger 仍是独立执行权威。
...
- managed-Skill 始终受当前 Tool、Sandbox、Enterprise Policy 和 Effect 治理；学习不能创造新执行权限。
...
每个 Action 都会根据目标、风险、可逆性、企业策略、当前 Effect 状态和 execution grant 独立判断。

Tool 调用不进入人工审批等待。Profile 作用域、Tool Policy、Enterprise Policy、Pi Active Tools、执行预算和 Effect 对账全部允许后，Tool 直接执行。Enterprise Policy 若显式要求人工审批，因为系统不存在审批工作流而 fail-closed。
...
Thruvera 只有一条 Context Pipeline 和一个 Compactor。
...
压缩后，Thruvera 会检查持久责任身份。缺失 Task 从权威 Task Ledger 恢复并写回 Pi Session transcript，不会从摘要中猜测。

## Initiative 与 Automation

Thruvera 可以观察持久 Trigger，并提议或执行有界主动工作。自治能力按证据门禁分级，不使用一个全局开关。
...
4. 没有任何 Adapter 能检查图片时，Thruvera 会明确失败。
...
OCR 可执行文件由宿主管理。若不在可信服务 `PATH` 中，请在宿主/服务环境设置绝对路径 `THRUVERA_LOCAL_OCR_COMMAND`；Profile YAML 与 Profile `.env` 不能选择可执行代码。

媒体结果以不可信证据进入 Pi，携带 digest、来源、置信度、告警和耗时。原始图片字节不会写入回执、遥测、Task Ledger 或 Memory。

## Skill、MCP、Web 与知识库

Thruvera 安装内置 Profile Skill，并渐进发现可用 Pi Skill。初始 Prompt 只包含 Skill 元数据，任务匹配后才加载完整正文。
...
将 `THRUVERA_WEKNORA_API_KEY` 保存在 Profile `.env` 中。
...
非敏感声明位于 `gateway.channels[]`。每个条目包含 Adapter ID、Instance ID、启用状态、`credentialRef` 和 Adapter Setting。内置渠道 Secret 保存在仅所有者可读的 Profile `.env`，并通过 `profile-env:<adapter>` 在可信 Adapter/诊断边界解析，不进入 YAML、普通 `ThruveraConfig`、日志、Memory 或模型上下文。
...
使用 `thruvera channel list --profile personal` 查看渠道声明，使用 `thruvera doctor --profile personal` 验证已启用 Adapter，再通过标准 Gateway 生命周期命令统一运行。

## 飞书与 Lark

Thruvera 支持飞书/Lark 自建应用，可使用 WebSocket 长连接或加密 webhook。
...
每个 Turn 使用一张流式交互卡呈现回答、进度、有界 Tool Activity 和用量元数据。Tool 调用不会显示审批按钮，也不会等待审批回复。同一时间只能有一个 Gateway 进程拥有一个 Profile。

飞书会议 Tool 支持会议查询、预约、参会人、主持人控制和录制生命周期。私有用户资源仍需要未来的飞书 User OAuth 层。

## Telegram

Thruvera 通过 Telegram Bot API 提供有界 long polling、默认拒绝的用户/聊天 allowlist、文本回复和编辑、typing 指示、原生图片/文件交付，以及对入站图片、文档、音频和语音的有界临时下载。
...
通过 BotFather 创建机器人，然后运行 `thruvera channel add telegram --profile personal`。Token 由安全提示输入或读取 `TELEGRAM_BOT_TOKEN`；允许的数字用户 ID 可在向导中输入或通过 `TELEGRAM_ALLOWED_USERS` 提供。
...
Docker 是 Thruvera 对内置命令和工作区 Tool 的首个生产 Execution Sandbox。可信本地执行不是 Sandbox。配置、强制限制、取消清理、能力作用域和真实 Docker 发布门禁详见 [Docker Execution Sandbox](docs/operations/docker-execution-sandbox.md)。
...
也可以使用 `thruvera service install --system` 安装系统级服务。请使用专用非 root 账户运行 Agent，并设置 `THRUVERA_SERVICE_USER`。
...
旧的 Actor-scoped 群聊 transcript 不会被猜测或自动合并。管理员可以通过 `thruvera migration session plan/apply` 将一个 transcript 显式分配给标准共享 Conversation，保留全部旧文件，并使用 digest-guarded rollback。详见 [Session 所有权迁移](docs/operations/session-ownership-migration.md)。

## CLI 参考

| 命令 | 用途 |
| --- | --- |
| `thruvera setup` | 配置 Profile、模型、身份、Skill 和可选渠道 |
| `thruvera chat` | 启动自适应本地终端 Agent |
| `thruvera gateway` | 配置、运行、安装、检查和控制 Channel Gateway |
| `thruvera binding` | 验证、解释、原子激活或禁用确定性 Channel-to-Profile Route |
| `thruvera profile` | 创建、选择、迁移、备份、检查和删除 Profile |
| `thruvera migration channel-instance` | 规划、执行、审计和安全回滚旧渠道 Route 所有权 |
| `thruvera model` | 查看或切换 Profile 模型 |
| `thruvera memory` | 检查、解释、编译、晋升、拒绝或遗忘 Memory 证据 |
| `thruvera autonomy` | 检查和控制证据门禁自治级别 |
| `thruvera credentials` | 管理加密 Profile Credential Vault |
| `thruvera effect` | 检查并对账未解决 Tool Effect |
| `thruvera trace` | 检查不含原始内容的 execution Trace |
| `thruvera doctor` | 验证 Runtime 和集成就绪状态 |
| `thruvera update` | 在保留 Profile 的同时安装最新已验证 Release |

运行 `thruvera --help` 查看完整命令。聊天中使用 `/help` 查看 Session、模型、压缩、Task、重试、取消和显示控制。

## 故障排查

### 机器人收不到消息

运行 `thruvera gateway health --profile <name>`。确认应用已发布、WebSocket 长连接已启用、已订阅 `im.message.receive_v1`，且发送者已被允许或配对。

### Task 重启后没有恢复

检查 `/tasks show <plan-id>`、`thruvera effect list --status unknown` 和 execution Trace。不安全或非幂等 Task 会 fail closed，不会重放。

### 纯文本模型无法读取图片

运行 `thruvera doctor`。配置支持图片的模型、启用辅助视觉，或确认 Tesseract 和所需语言数据已安装。
...
运行 `thruvera mcp status --profile <name>`。检查 Server 命令或 URL、所需环境变量、启动 Deadline 和 Profile Toolset。
...
Tag、根包、所有 Thruvera Workspace、内部依赖和 Changelog 版本必须一致。归档 Verifier 会检查跨平台校验和、源码布局、隔离安装、重建、Profile reload 和已打包 Skill。
...
- [统一 Agent Runtime PRD](docs/prd/thruvera-pi-unified-agent-runtime.md)
...
Thruvera v1.5.1 不包含固定客户业务本体、第二个 Agent Loop、高风险全自治执行、大型多 Agent 组织或任意模型生成的生产 Skill 变更。
